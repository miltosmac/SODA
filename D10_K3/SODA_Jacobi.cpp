...
void jacobi9d_soda(
	...
	mat ff_edge_0[5],ff_edge_k[5],start[k],ff_inter_a[5],result[k]; //={0}
...
#pragma HLS UNROLL
			//REUSE CHAIN k-1 (v=0 ||v=k)
				if (v==1){ //The First and Last Reuse Chain have a different layout than the rest
					//ff_edge_0[2]=fifo_edge_01[FIFO_WIDTH-2];
			...
			result[2]=coeff*(start[0]+ff_edge_0[1]+ff_edge_0[3]+ff_inter_a[0]+ff_inter_a[2]+ff_inter_a[4]+start[2]+ff_edge_k[1]+ff_edge_k[3]);

			result[0]=coeff*(start[2]+ff_edge_k[1]+ff_edge_k[3]+start[1]+ff_inter_a[1]+ff_inter_a[3]+start[0]+ff_edge_0[1]+ff_edge_0[3]);

			result[1]=coeff*(ff_edge_0[0]+ff_edge_0[2]+ff_edge_0[4]+ff_inter_a[0]+ff_inter_a[2]+ff_inter_a[4]+start[2]+ff_edge_k[1]+ff_edge_k[3]);

			if (i==0){ //HALO First Line
				if (j==-1){
					out2.write(ff_edge_k[1]);
				...
				if (j==-1){//HALO elements
					//if(i>1){ //For i==1 the HALO(i-1,j=WIDTH-2) was already written to out1
						out1.write(ff_inter_a[2]);//HALO(i-1,j=WIDTH-2)
					//}
					out2.write(ff_edge_k[1]);//HALO (i,j=-1)
					out0.write(result[0]);
				}
				else{
					out0.write(result[0]);
